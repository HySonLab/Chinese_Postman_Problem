// Directed Evolution (DE) with multi-threading for the load-dependent Chinese postman problem
...
// For multi-threading
static void Method_k_MOVE(
	Graph *graph, 
	const vector<Edge> &sigma, 
	const vector<int> &k_indices, 
	...
	for (int i = 0; i < k_indices.size(); ++i) {
		...
		A = B;
		result.second = B_value;
	}

	result.first = A;
	assert(result.first.size() == sigma.size());
}
...
pair< vector<Edge>, double> Directed_Evolution_MultiThreads(
	...
	// Multi-threading
	std::thread threads[num_variants];
    ...
    for (int iter = 1; iter <= num_iterations; ++iter) {
		...
		vector< pair< vector<Edge>, double > > variants;
		variants.clear();
		for (int variant = 0; variant < num_variants; ++variant) {
			pair< vector<Edge>, double> obj;
			variants.push_back(obj);
		}

		for (int variant = 0; variant < num_variants; ++variant) {
       		threads[variant] = std::thread(Method_k_MOVE, graph, std::cref(sigma), std::cref(all_indices[variant]), std::ref(variants[variant]));
		}
		...
		// Update
		for (int variant = 0; variant < num_variants; ++variant) {
			if (variants[variant].second < best) {
				best = variants[variant].second;
				sigma_star = variants[variant].first;
				improve = true;
			}
		}
